...
public class InfoDAO {
	...
	public void setInfoRegiste(InfoVO ivo) throws Exception {
		...
		try {
			...
			pstmt.setString(5, ivo.getReservation_date());
	...
	public void setInfoUpdate(InfoVO ivo) throws Exception {
		String sql = "update reservationinfo set reservation_pw=?, phone=?, reservation_date=? where reservation_id=?";
		Connection con = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			// 중복 확인
			if (getInfoIdOverlap(ivo.getReservation_id())) {
				return;
			}
			...
			pstmt.setString(3, ivo.getReservation_date());
			pstmt.setString(4, ivo.getReservation_id());
			...
			System.out.println("데이터베이스 오류: 중복된 정보가 이미 존재합니다.");
			se.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public InfoVO getInfo(String id, String pw) throws Exception {
		...
		try {
			...
			System.out.println("일련번호\t예약자ID\t\t예약자PW\t\t성함\t 연락처\t\t\t예약일");
			...
			if (rs.next()) {
				...
				String reservation_date = rs.getString("reservation_date").split(" ")[0]; // 날짜 뒤에 00 삭제
				ivo.setReservation_date(reservation_date);
				System.out.println(ivo.getNo() + "\t" + ivo.getReservation_id() + "\t\t" + ivo.getReservation_pw()
						+ "\t\t" + ivo.getName() + "\t" + ivo.getPhone() + "\t\t" + ivo.getReservation_date());
			}
